...
const Field = ({ children, style={} }) => (
  <div style={{ marginBottom:22, ...style }}>{children}</div>
);

const Row = ({ children, cols, gap=16 }) => (
  <div style={{ display:"grid", gridTemplateColumns: cols || "repeat(auto-fit,minmax(160px,1fr))", gap }}>
    {children}
  </div>
);
...
const Note = ({ children }) => (
  <div style={{ background:G.pale, border:`1px solid ${G.border}`, borderRadius:8,
    padding:"12px 16px", fontFamily:"'Barlow',sans-serif", fontSize:13,
    color:G.mid, marginBottom:22, display:"flex", gap:10, alignItems:"flex-start" }}>
    <span style={{ fontSize:16 }}>ℹ️</span><span>{children}</span>
  </div>
);
...
const SubHeading = ({ children, optional }) => (
  <p style={{ margin:"0 0 14px", fontFamily:"'Barlow Condensed',sans-serif", fontWeight:700,
    ...
    {children}{optional && <span style={{ fontWeight:400, color:G.muted, textTransform:"none", letterSpacing:0 }}> (optional)</span>}
  </p>
);

const Divider = () => (
  <div style={{ borderTop:`1px solid ${G.border}`, margin:"24px 0" }} />
);

const STEPS = [
  "Contact Info","Site Access","Fan Details",
  "Installation","Controls","Electrical","Review & Sign",
];

const defaultForm = {
  companyName:"",
  siteStreet:"", siteCity:"", siteState:"", siteZip:"",
  ...
  accessTimes:"", afterHours:"no", preferredMonths:"",
  safetyBrief:"no", safetyDuration:"",
  ppe:"no", ppeList:"",
  paperwork:"no", paperworkList:"",
  fanTypes:{ TITAN:false,ECO:false,XP:false,DDI:false,TRAK_EDGE:false },
  fanQty:"",
  fanDiameters:{ "5ft":false,"6ft":false,"7ft":false,"8ft":false,"10ft":false,
    "12ft":false,"14ft":false,"16ft":false,"18ft":false,"20ft":false,"24ft":false },
  fanVoltage:{ "120V/1PH":false,"240V/1PH":false,"240V/3PH":false,"480V/3PH":false },
  downrod:"",
...
export default function App() {
  ...
  const validate = () => {
    ...
    if (step === 2) {
      if (!Object.values(form.fanTypes).some(Boolean))     e.fanTypes     = "Select at least one fan type";
      if (!form.fanQty.trim())                             e.fanQty       = "Required";
      if (!Object.values(form.fanDiameters).some(Boolean)) e.fanDiameters = "Select at least one diameter";
      if (!Object.values(form.fanVoltage).some(Boolean))   e.fanVoltage   = "Select voltage/phase";
      if (!form.downrod)                                   e.downrod      = "Select a downrod type";
    }
    if (step === 3) {
      if (!form.ceilingHeight.trim()) e.ceilingHeight    = "Required";
      if (!form.ceilingStructure)     e.ceilingStructure = "Select ceiling structure";
    }
    if (step === 4) {
  ...
  // ─────────────────────────────────────────────────────────────
  // Steps are called as plain functions — NOT rendered as <Components />.
  // This is the critical fix for iOS: when a component is defined inside
  // App and used as <Comp />, React sees a new type each render and
  // unmounts + remounts the DOM node, killing the mobile keyboard.
  // Calling renderStepN() keeps the same DOM nodes alive across re-renders.
  // ─────────────────────────────────────────────────────────────
  ...
  const renderStep0 = () => (
    <>
      ...
      <Row>
        <Field>
          <Label required>Hunter Industrial Sales Representative</Label>
      ...
      <Field>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
          {[
            ["primary",   "I am the Primary On-Site Contact"],
            ["secondary", "I am the Secondary On-Site Contact"],
      ...
      <Row>
        <Field><Label>Phone Number</Label><Input type="tel" value={form.secondaryPhone} onChange={set("secondaryPhone")} /></Field>
  ...
  const renderStep1 = () => {
    ...
    return (
      <>
          ...
          <div style={{ display:"flex", flexWrap:"wrap", gap:8 }}>
            {days.map(d => (
              <label key={d} style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:6,
                ...
                <input type="checkbox" checked={form.accessDays[d]}
                  onChange={setNested("accessDays",d)}
        ...
        {[
          ["safetyBrief","Safety or security brief required?","safetyDuration","Duration of brief"],
          ["ppe","PPE required?","ppeList","List required PPE"],
          ["paperwork","Paperwork or forms required for site access?","paperworkList","List required forms (COI, Certifications, etc.)"],
        ].map(([key,label,condKey]) => (
          <Field key={key}>
            <Label>{label}</Label>
            <div style={{ display:"flex", gap:10, marginBottom: form[key]==="yes" ? 12 : 0 }}>
              <RadioOpt name={key} label="Yes" value="yes" checked={form[key]==="yes"} onChange={() => setVal(key,"yes")} />
              <RadioOpt name={key} label="No"  value="no"  checked={form[key]==="no"}  onChange={() => setVal(key,"no")} />
            </div>
            {form[key]==="yes" && <Input value={form[condKey]} onChange={set(condKey)} />}
          </Field>
        ))}
      </>
    );
  ...
  const renderStep2 = () => {
    const fans = ["TITAN","ECO","XP","DDI","TRAK_EDGE"];
    const fanLabels = { TITAN:"TITAN",ECO:"ECO",XP:"XP",DDI:"DDI",TRAK_EDGE:"TRAK EDGE" };
    ...
    const downrods = [
      ...
      "Rigid (Fixed): 2 ft","Rigid (Fixed): 3 ft","Rigid (Fixed): 4 ft",
    ];
    return (
      <>
        <SectionTitle>Fan Details & Mounting</SectionTitle>
        <Field>
          <Label required>Fan Model(s) to be Installed</Label>
          <div style={{ display:"flex", flexWrap:"wrap", gap:10 }}>
            {fans.map(f => (
              <CheckOpt key={f} label={fanLabels[f]} checked={form.fanTypes[f]} onChange={setNested("fanTypes",f)} />
            ))}
          </div>
          <ErrorMsg msg={errors.fanTypes} />
        </Field>

        <Field style={{ maxWidth:200 }}>
          <Label required>Total Number of Fans</Label>
          <Input type="number" value={form.fanQty} onChange={set("fanQty")} error={errors.fanQty} />
          <ErrorMsg msg={errors.fanQty} />
        </Field>

        <Field>
          <Label required>Fan Diameter(s)</Label>
          <div style={{ display:"flex", flexWrap:"wrap", gap:8 }}>
            {diams.map(d => (
              <CheckOpt key={d} label={d.replace("ft"," ft.")} checked={form.fanDiameters[d]} onChange={setNested("fanDiameters",d)} />
            ))}
          </div>
          <ErrorMsg msg={errors.fanDiameters} />
        </Field>
        ...
        <Field>
          ...
          <div style={{ display:"flex", flexWrap:"wrap", gap:8 }}>
            {volts.map(v => (
              <CheckOpt key={v} label={v} checked={form.fanVoltage[v]} onChange={setNested("fanVoltage",v)} />
            ))}
          </div>
          <ErrorMsg msg={errors.fanVoltage} />
        </Field>
        ...
        <Field>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
            {downrods.map(d => (
              <RadioOpt key={d} name="downrod" label={d} value={d} checked={form.downrod===d} onChange={() => setVal("downrod",d)} />
            ))}
          </div>
          <ErrorMsg msg={errors.downrod} />
        </Field>
  ...
  const renderStep3 = () => {
    const structures = ["I-Beam","Glulam Beam","Wood Beam","Steel Truss","Z-Purlin"];
    ...
    return (
      <>
        ...
        <Field>
          <Label>Will Hunter need to provide a lift?</Label>
          ...
          <div style={{ display:"flex", gap:10, marginBottom:16 }}>
            <RadioOpt name="hunterLift" label="Yes — Hunter provides" value="yes" checked={form.hunterLift==="yes"} onChange={() => setVal("hunterLift","yes")} />
          ...
          <div style={{ display:"flex", flexWrap:"wrap", gap:8 }}>
            {lifts.map(l => (
              <RadioOpt key={l} name="liftType" label={l} value={l} checked={form.liftType===l} onChange={() => setVal("liftType",l)} />
            ))}
          </div>
        </Field>
        ...
        <Field>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
            {structures.map(s => (
              <RadioOpt key={s} name="ceilingStructure" label={s} value={s} checked={form.ceilingStructure===s} onChange={() => setVal("ceilingStructure",s)} />
            ))}
          </div>
          <ErrorMsg msg={errors.ceilingStructure} />
          <div style={{ marginTop:12 }}>
            <Label>Other (if not listed above)</Label>
            <Input value={form.ceilingOther} onChange={set("ceilingOther")} />
          </div>
        </Field>

        <Field>
          <Label>Any unmovable obstructions under fan mounting locations?</Label>
          <div style={{ display:"flex", gap:10, marginBottom: form.obstructions==="yes"?12:0 }}>
          ...
          {form.obstructions==="yes" && <Input value={form.obstructionDetails} onChange={set("obstructionDetails")} />}
        </Field>

        <Field>
          <Label>Existing fans to be removed?</Label>
          <div style={{ display:"flex", flexWrap:"wrap", gap:8 }}>
            {[["no","No"],["hvls","Yes — Industrial HVLS Fan"],["commercial","Yes — Commercial Fan"]].map(([v,l]) => (
              <RadioOpt key={v} name="existingFans" label={l} value={v} checked={form.existingFans===v} onChange={() => setVal("existingFans",v)} />
            ))}
          </div>
        </Field>
  ...
  const renderStep4 = () => {
    const controllers = Object.keys(form.controllerTypes);
    return (
      <>
        ...
        <Field>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))", gap:8 }}>
            {controllers.map(c => (
              <CheckOpt key={c} label={c} checked={form.controllerTypes[c]} onChange={setNested("controllerTypes",c)} />
            ))}
          </div>
        ...
        <Field>
          ...
          <div style={{ display:"flex", gap:10, marginBottom: form.distanceExceeds==="yes"?12:0 }}>
          ...
          {form.distanceExceeds==="yes" && (
            <div style={{ display:"flex", flexWrap:"wrap", gap:8 }}>
              {["100–200 ft","200–300 ft","Greater than 300 ft"].map(r => (
                <RadioOpt key={r} name="distanceRange" label={r} value={r} checked={form.distanceRange===r} onChange={() => setVal("distanceRange",r)} />
              ))}
            </div>
          )}
        </Field>
        ...
        <Field>
          ...
          <div style={{ display:"flex", flexWrap:"wrap", gap:8 }}>
            {[["not_chained","Not Daisy-Chained"],["lt100","< 100 ft"],["lt200","< 200 ft"],["lt300","< 300 ft"],["gt300","> 300 ft"]].map(([v,l]) => (
              <RadioOpt key={v} name="daisyChain" label={l} value={v} checked={form.daisyChain===v} onChange={() => setVal("daisyChain",v)} />
            ))}
          </div>
        </Field>
  ...
  const renderStep5 = () => (
    <>
      ...
      <Field>
        ...
        <div style={{ display:"flex", flexWrap:"wrap", gap:10 }}>
          {[["yes","Yes"],["no","No"],["unsure","Unsure"]].map(([v,l]) => (
            <RadioOpt key={v} name="spareBreakers" label={l} value={v} checked={form.spareBreakers===v} onChange={() => setVal("spareBreakers",v)} />
          ))}
        </div>
      </Field>
      ...
      {form.spareBreakers==="yes" && (
        <Field>
          <Label>How many spare breakers and what amperage?</Label>
      ...
      <Field>
        ...
        <div style={{ display:"flex", gap:10, marginBottom: form.conduitRequired==="yes"?12:0 }}>
        ...
        {form.conduitRequired==="yes" && <Input value={form.conduitType} onChange={set("conduitType")} />}
      </Field>

      <Field>
        <Label>Will Hunter be connecting fire wire to fans?</Label>
  ...
  const renderStep6 = () => {
    const siteAddress = [form.siteStreet, form.siteCity, form.siteState, form.siteZip].filter(Boolean).join(", ");
    const summary = [
      ["Company",      form.companyName],
      ["Site Address", siteAddress || "—"],
      ["Sales Rep",    form.salesRep],
      ["Quote #",      form.quoteNumber],
      ["Primary",      `${form.primaryFirst} ${form.primaryLast}`],
      ["Phone",        form.primaryPhone],
      ["Email",        form.primaryEmail],
      ["Fan Models",   Object.entries(form.fanTypes).filter(([,v])=>v).map(([k])=>k.replace("TRAK_EDGE","TRAK EDGE")).join(", ")||"—"],
      ["Fan Qty",      form.fanQty||"—"],
      ["Diameter(s)",  Object.entries(form.fanDiameters).filter(([,v])=>v).map(([k])=>k.replace("ft"," ft.")).join(", ")||"—"],
      ["Voltage",      Object.entries(form.fanVoltage).filter(([,v])=>v).map(([k])=>k).join(", ")||"—"],
      ["Downrod",      form.downrod||"—"],
    ...
    return (
      <>
        ...
        <Field>
          ...
          <Textarea value={form.additionalNotes} onChange={set("additionalNotes")} />
        </Field>

        <div style={{ background:"#FFF9E6", border:`1.5px solid ${G.gold}`, borderRadius:10, padding:18, marginBottom:18 }}>
          <p style={{ margin:"0 0 10px", fontFamily:"'Barlow',sans-serif", fontWeight:700,
            fontSize:12, color:"#7A5C00", textTransform:"uppercase", letterSpacing:"0.06em" }}>Permitting Acknowledgement</p>
          <p style={{ margin:"0 0 14px", fontFamily:"'Barlow',sans-serif", fontSize:13, color:G.text, lineHeight:1.6 }}>
            All federal, state, and city permitting, structural engineering review, letters, approvals, and fire panel terminations are solely the responsibility of the building owner or purchasing party.
          </p>
        ...
        <div style={{ background:"#F0F7F0", border:`1.5px solid ${G.border}`, borderRadius:10, padding:18, marginBottom:24 }}>
          <p style={{ margin:"0 0 10px", fontFamily:"'Barlow',sans-serif", fontWeight:700,
            fontSize:12, color:G.mid, textTransform:"uppercase", letterSpacing:"0.06em" }}>Mechanical Installation Note</p>
          <p style={{ margin:"0 0 14px", fontFamily:"'Barlow',sans-serif", fontSize:13, color:G.text, lineHeight:1.6 }}>
            Power must be available and in place for fans to be tested upon completion of installation. No return trips for testing or troubleshooting. Additional charges will apply if return trips are requested.
          </p>
          <label style={{ display:"flex", alignItems:"flex-start", gap:10, cursor:"pointer" }}>
        ...
        <p style={{ fontFamily:"'Barlow',sans-serif", fontSize:12, color:G.muted, marginTop:-8 }}>
          By entering your name you agree to Hunter Industrial's{" "}
          <a href="https://industrialfans.hunterfan.com/pages/privacy" target="_blank" rel="noopener" style={{ color:G.mid }}>Privacy Policy & Terms</a>.
        </p>
      </>
    );
  };
  ...
  if (submitted) return (
    <div style={{ minHeight:"100vh", background:G.dark, display:"flex",
      ...
      <div style={{ background:G.white, borderRadius:16, padding:"48px 40px", maxWidth:520, width:"100%", textAlign:"center" }}>
        ...
        <h1 style={{ fontFamily:"'Barlow Condensed',sans-serif", fontWeight:800,
          fontSize:28, color:G.dark, margin:"0 0 12px", textTransform:"uppercase" }}>Form Submitted!</h1>
        <p style={{ fontFamily:"'Barlow',sans-serif", color:G.muted, fontSize:15, lineHeight:1.7, margin:"0 0 24px" }}>
          Thank you, <strong>{form.signatureName}</strong>. Your site assessment for <strong>{form.companyName}</strong> has been received.
          Your Hunter Industrial representative <strong>{form.salesRep}</strong> will follow up shortly regarding order <strong>{form.quoteNumber}</strong>.
        </p>
        <p style={{ fontFamily:"'Barlow',sans-serif", color:G.muted, fontSize:13 }}>
          Questions? Call <strong>1-844-591-FANS (3267)</strong>
        </p>
        <button onClick={() => { setSubmitted(false); setStep(0); setForm(defaultForm); }}
          style={{ marginTop:24, padding:"12px 28px", borderRadius:8,
            ...
            color:G.mid, cursor:"pointer" }}>Submit Another Form</button>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#F4F7F4" }}>
      <div style={{ background:`linear-gradient(135deg,${G.dark} 0%,${G.mid} 100%)`,
        padding:"20px 24px", display:"flex", alignItems:"center", justifyContent:"space-between",
        boxShadow:"0 2px 12px rgba(0,0,0,.3)" }}>
        <div>
          <div style={{ fontFamily:"'Barlow Condensed',sans-serif", fontWeight:800,
            fontSize:22, color:G.white, letterSpacing:"0.08em", textTransform:"uppercase" }}>Hunter Industrial</div>
          <div style={{ fontFamily:"'Barlow',sans-serif", fontSize:12,
            color:"rgba(255,255,255,.65)", letterSpacing:"0.1em", textTransform:"uppercase" }}>Site Assessment Form</div>
        </div>
        <div style={{ textAlign:"right", fontSize:11, color:"rgba(255,255,255,.5)",
          fontFamily:"'Barlow',sans-serif", lineHeight:1.5 }}>
          Smyrna, TN 37167<br/>1-844-591-FANS
        </div>
      </div>
      ...
      <div style={{ maxWidth:760, margin:"0 auto", padding:"32px 20px 60px" }}>
        ...
        <div style={{ marginBottom:32 }}>
            ...
            {STEPS.map((s, i) => (
              <button key={i} onClick={() => { if (i < step) { setErrors({}); setStep(i); } }}
                style={{ flexShrink:0, padding:"6px 14px", borderRadius:20,
                  ...
                  background: i===step ? G.mid : i<step ? G.pale : "#E0E0E0",
          ...
          <div style={{ height:4, background:"#E0E8E1", borderRadius:2 }}>
            <div style={{ height:"100%", width:`${((step+1)/STEPS.length)*100}%`,
              background:`linear-gradient(90deg,${G.mid},${G.accent})`,
        ...
        <div style={{ background:G.white, borderRadius:14, padding:"32px 28px",
          ...
          {/* Called as function — not JSX component — to preserve DOM across re-renders */}
          {stepRenderers[step]()}
            ...
            {step > 0
              ? <button onClick={back}
                  style={{ padding:"12px 28px", borderRadius:8, border:`2px solid ${G.border}`,
                    ...
                    fontSize:14, color:G.mid, cursor:"pointer", letterSpacing:"0.05em" }}>← Back</button>
              : <div />}
            <button onClick={step === STEPS.length-1 ? submit : next}
              style={{ padding:"12px 36px", borderRadius:8, border:"none",
                background:`linear-gradient(135deg,${G.mid},${G.dark})`, color:G.white,
                fontFamily:"'Barlow',sans-serif", fontWeight:700, fontSize:14, cursor:"pointer",
                letterSpacing:"0.05em", boxShadow:"0 2px 8px rgba(45,90,46,.3)" }}>
